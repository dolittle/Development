...
const blackList = ['dolittle-einar', 'Datadolittle', 'dolittle-obsolete', 'dolittle-technolog'];
const whiteList =
  [
    ...
    'dolittle-contribs', 'dolittle-actions'
  ];
...
async function GetOrganisationDetails(orgLogin: string): Promise<any> {
  const orgs = (await octokit.orgs.get({ org: orgLogin })).data
...
async function GetRepositoriesForOrganisation(orgLogin: string): Promise<any[]> {
  const repos = (await octokit.repos.listForOrg({ org: orgLogin })).data;
...
(async () => {
  ...
  for (let org of orgDetailList) {
    let repos = await GetRepositoriesForOrganisation(org.login);
    if (repos.length) {
      // console.log(`${org.login} has ${repos.length} repos`);
    } else {
      console.log(`${org.login} has no repos, pls deleterino`);
    }
  }
